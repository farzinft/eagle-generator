...
use Eagle\Generator\Utils\FileUtil;
...
class VueJsLayoutPublishCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'eagle.publish:vuejs';
    ...
    {
        $viewsPath = config('eagle.eagle_generator.path.views', base_path('resources/views/'));
        $resourcesPath = config('eagle.eagle_generator.path.resourcesPath', base_path('resources/'));
        $vendorPath = $resourcesPath.'assets/vendor/';
        $assetsJsPath = config('eagle.eagle_generator.path.assetsJsPath', base_path('resources/assets/js/'));
        $assetsCssPath = config('eagle.eagle_generator.path.assetsCssPath', base_path('resources/assets/css/'));
        $templateType = config('eagle.eagle_generator.templates', 'core-templates');
        $requestPath = config('eagle.eagle_generator.path.api_request', base_path('app/Http/Requests/'));
        ...
        foreach ($files as $stub => $blade) {
            $sourceFile = base_path('vendor/eaglelabs/'.$templateType.'/templates/'.$stub);
            $destinationFile = $viewsPath.$blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($vueLayoutFiles as $stub => $blade) {
            $sourceFile = base_path('vendor/eaglelabs/'.$templateType.'/templates/vuejs/'.$stub);
            $destinationFile = $viewsPath.$blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($baseRequestCustomFiles as $stub => $php) {
            $sourceFile = base_path('vendor/eaglelabs/laravel-generator/templates/'.$stub);
        ...
        $sourceFile = base_path('vendor/eaglelabs/laravel-generator/templates/vuejs/js/gulpfile.js');
        $destinationFile = base_path().'/gulpfile.js';
        $this->publishFile($sourceFile, $destinationFile, 'gulpfile.js');

        $sourceFile = base_path('vendor/eaglelabs/laravel-generator/templates/vuejs/js/package.json');
    ...
    {
        $path = config('eagle.eagle_generator.path.routes', app_path('Http/routes.php'));
    ...
    private function publishHomeController()
    {
        ...
        $controllerPath = config('eagle.eagle_generator.path.controller', app_path('Http/Controllers/'));
    ...
    private function fillTemplate($templateData)
    {
        $templateData = str_replace(
            ...
            config('eagle.eagle_generator.namespace.controller'), $templateData
        );

        $templateData = str_replace(
            '$NAMESPACE_REQUEST$',
            config('eagle.eagle_generator.namespace.request'), $templateData
        );
