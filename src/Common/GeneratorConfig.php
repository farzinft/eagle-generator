...
namespace Eagle\Generator\Common;
...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('eagle.eagle_generator.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('eagle.eagle_generator.namespace.model', 'App\Models').$prefix;
        if (config('eagle.eagle_generator.ignore_model_prefix', false)) {
            $this->nsModel = config('eagle.eagle_generator.namespace.model', 'App\Models');
        }
        $this->nsDataTables = config('eagle.eagle_generator.namespace.datatables', 'App\DataTables').$prefix;
        $this->nsModelExtend = config(
            'eagle.eagle_generator.model_extend_class',
            'Illuminate\Database\Eloquent\Model'
        );

        $this->nsApiController = config(
            'eagle.eagle_generator.namespace.api_controller',
            'App\Http\Controllers\API'
        ).$prefix;
        $this->nsApiRequest = config('eagle.eagle_generator.namespace.api_request', 'App\Http\Requests\API').$prefix;

        $this->nsRequest = config('eagle.eagle_generator.namespace.request', 'App\Http\Requests').$prefix;
        $this->nsRequestBase = config('eagle.eagle_generator.namespace.request', 'App\Http\Requests');
        $this->nsBaseController = config('eagle.eagle_generator.namespace.controller', 'App\Http\Controllers');
        $this->nsController = config('eagle.eagle_generator.namespace.controller', 'App\Http\Controllers').$prefix;
    }
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
            'eagle.eagle_generator.path.repository',
            app_path('Repositories/')
        ).$prefix;

        $this->pathModel = config('eagle.eagle_generator.path.model', app_path('Models/')).$prefix;
        if (config('eagle.eagle_generator.ignore_model_prefix', false)) {
            $this->pathModel = config('eagle.eagle_generator.path.model', app_path('Models/'));
        }

        $this->pathDataTables = config('eagle.eagle_generator.path.datatables', app_path('DataTables/')).$prefix;

        $this->pathApiController = config(
            'eagle.eagle_generator.path.api_controller',
            app_path('Http/Controllers/API/')
        ).$prefix;

        $this->pathApiRequest = config(
            'eagle.eagle_generator.path.api_request',
            app_path('Http/Requests/API/')
        ).$prefix;

        $this->pathApiRoutes = config('eagle.eagle_generator.path.api_routes', app_path('Http/api_routes.php'));

        $this->pathApiTests = config('eagle.eagle_generator.path.api_test', base_path('tests/'));

        $this->pathApiTestTraits = config('eagle.eagle_generator.path.test_trait', base_path('tests/traits/'));

        $this->pathController = config(
            'eagle.eagle_generator.path.controller',
            app_path('Http/Controllers/')
        ).$prefix;

        $this->pathRequest = config('eagle.eagle_generator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('eagle.eagle_generator.path.routes', app_path('Http/routes.php'));
        ...
        $this->modelJsPath = config(
                'eagle.eagle_generator.path.modelsJs',
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('eagle.eagle_generator.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('eagle.eagle_generator.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        $this->options['softDelete'] = config('eagle.eagle_generator.options.softDelete', false);
    ...
    {
        $this->prefixes['route'] = explode('/', config('eagle.eagle_generator.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('eagle.eagle_generator.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('eagle.eagle_generator.prefixes.view', ''));
        $this->prefixes['public'] = explode('/', config('eagle.eagle_generator.prefixes.public', ''));
    ...
    {
        $this->addOns['swagger'] = config('eagle.eagle_generator.add_on.swagger', false);
        $this->addOns['tests'] = config('eagle.eagle_generator.add_on.tests', false);
        $this->addOns['datatables'] = config('eagle.eagle_generator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('eagle.eagle_generator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('eagle.eagle_generator.add_on.menu.menu_file', 'layouts.menu');
    }
}
